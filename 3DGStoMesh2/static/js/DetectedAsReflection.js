...
const cmpButtonRight = document.getElementById("DRCmpButtonRight");
const cmpButtonLeft = document.getElementById("DRCmpButtonLeft");
const cmpContainerLeft = document.getElementById("DRCmpContainerLeft");
const cmpContainerRight = document.getElementById("DRCmpContainerRight");
const modelLabelDropdown = document.getElementById("DRModelLabelDropdown");
const check3dgsUrl = document.getElementById("DRCheck3dgs");
...
let modelList = {
  BackMirror: {
    url: "https://www.kiriengine.app/share/3dgs?taskId=1853434364776415232",
    distance: 0,
  },
  Buddha: {
    url: "https://www.kiriengine.app/share/3dgs?taskId=1853635484354347008",
    distance: 0,
  },
  TV: {
    url: "https://www.kiriengine.app/share/3dgs?taskId=1855989574556188672",
    distance: 0,
  },
};

let modelType = 0;
let modelTypeList = ["2.0 wo reflection", "1.0", "photo"];
...
modelLabelDropdown.addEventListener("change", (event) => {
  ...
  loadObj(
    `DetectedAsReflection/${model}/${modelTypeList[modelType]}/3DModel`,
    scene2,
    (p) => {
...
function handleDRComparisonClick(name) {
  model = name;
  ...
  renderNextFrame = true;
  cmpLeftLoad(`DetectedAsReflection/${name}/2.0 w reflection/3DModel`, name);
  scene2.remove(object2);
  renderNextFrame = true;
  cmpRightLoad(`DetectedAsReflection/${name}/${modelTypeList[modelType]}/3DModel`, name);
  renderNextFrame = true;
}

window.handleDRComparisonClick = handleDRComparisonClick;

cmpLeftLoad(`DetectedAsReflection/${model}/2.0 w reflection/3DModel`, model);
cmpRightLoad(`DetectedAsReflection/${model}/2.0 wo reflection/3DModel`, model);
animate();
