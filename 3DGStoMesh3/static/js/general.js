...
let modelList = {
  Gundam: {
    url: "https://test.kiriengine.app/share/3dgs?taskId=2034493560828461056",
    distance: 0,
  },
  Windowsill: {
    url: "https://test.kiriengine.app/share/3dgs?taskId=2041790586331070464",
    distance: 0,
  },
  StoneElephant: {
    url: "https://test.kiriengine.app/share/3dgs?taskId=2041780146960072704",
    distance: 0,
  },
};
...
function cmpLeftLoad(url, name) {
  ...
  if (texture1 && Array.isArray(texture1)) {
    texture1.forEach((item) => {
      item?.dispose();
    });
  } else {
    texture1?.dispose();
      ...
      if (Array.isArray(child.material)) {
        if (!texture1) {
          ...
          child.material.forEach((item) => {
            texture1.push(item.map);
          });
        }
        child.material.forEach((item, index) => {
          if (isVisible) {
            item.map = texture1[index];
          } else {
            item.map = null;
          }
    ...
    if (child.isMesh && child.material) {
      if (Array.isArray(child.material)) {
        if (!texture2) {
          texture2 = [];
          child.material.forEach((item) => {
            texture2.push(item.map);
          });
        }
        child.material.forEach((item, index) => {
          if (isVisible) {
            item.map = texture2[index];
          } else {
            item.map = null;
          }
          item.needsUpdate = true;
...
function handleGeneralComparisonClick(name) {
  ...
  renderNextFrame = true;
  cmpLeftLoad(`General/${name}/3.0/3DModel`, name);
  scene2.remove(object2);
  renderNextFrame = true;
  cmpRightLoad(`General/${name}/2.0/3DModel`, name);
  renderNextFrame = true;
}

window.handleGeneralComparisonClick = handleGeneralComparisonClick;

cmpLeftLoad(`General/${model}/3.0/3DModel`, model);
cmpRightLoad(`General/${model}/2.0/3DModel`, model);
animate();
